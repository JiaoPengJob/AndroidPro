...
/**
 * 关于加密填充方式：之前以为上面这些操作就能实现rsa加解密，以为万事大吉了，呵呵，这事还没完，悲剧还是发生了，
 * Android这边加密过的数据，服务器端死活解密不了，原来android系统的RSA实现是"RSA/None/NoPadding"，
 * 而标准JDK实现是"RSA/None/PKCS1Padding" ，这造成了在android机上加密后无法在服务器上解密的原因，
 * 所以在实现的时候这个一定要注意。
 *
 * 实现分段加密：搞定了填充方式之后又自信的认为万事大吉了，可是意外还是发生了，
...
/**
 * Created by 马超龙 on 2017/10/18.
 * RSA加密解密操作
 * 参考博客http://www.cnblogs.com/whoislcj/p/5470095.html
 */
...
public class RSAmcl {
    ...
    public static byte[] encryptByPublicKey(byte[] data, byte[] publicKey) throws Exception {
        ...
        // 加密数据
        Cipher cp = Cipher.getInstance(ECB_PKCS1_PADDING,"BC");
