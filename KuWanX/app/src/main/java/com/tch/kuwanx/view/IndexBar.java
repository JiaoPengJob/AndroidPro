...
import com.tch.kuwanx.bean.BaseIndexPinyinBean;
...
public class IndexBar extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(measureWidth, measureHeight + 4);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mIndexDatas.size(); i++) {
            ...
            mPaint.getTextBounds(index, 0, index.length(), indexBounds);//测量计算文字所在矩形，可以得到宽高
            Paint
                    .FontMetrics fontMetrics = mPaint.getFontMetrics();//获得画笔的FontMetrics，用来计算baseLine。因为drawText的y坐标，代表的是绘制的文字的baseLine的位置
            int baseline = (int) ((mGapHeight - fontMetrics.bottom - fontMetrics.top) / 2);//计算出在每格index区域，竖直居中的baseLine值
            canvas.drawText(index, mWidth / 2 - indexBounds.width() / 2, t + mGapHeight * i + baseline + 2, mPaint);//调用drawText，居中显示绘制index
        }
    }
    ...
    private void initSourceDatas() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int i1 = 0; i1 < target.length(); i1++) {
                ...
                //如果c不是汉字，则返回String.valueOf(c)d
                if (Pinyin.isChinese(target.charAt(i1))) {
                    pySb.append(Pinyin.toPinyin(target.charAt(i1)));
                } else {
                    String newStr = String.valueOf(target.charAt(i1)).toUpperCase();
                    pySb.append(Pinyin.toPinyin(newStr.charAt(0)));
                }
            }
            indexPinyinBean.setPyIndex(pySb.toString());//设置姓名全拼音
        ...
        Collections.sort(mSourceDatas, new Comparator<BaseIndexPinyinBean>() {
            ...
            public int compare(BaseIndexPinyinBean lhs, BaseIndexPinyinBean rhs) {
                if (lhs.getTag().equals("#")) {
                    ...
                    return lhs.getPyIndex().compareTo(rhs.getPyIndex());
                }
            }
        });
    }
